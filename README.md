...
이 문서에서는 Jvault라이브러리를 프로젝트에 다운하는 방법부터 간략한 Jvault사용방법 등 을 소개합니다.   
__실제 사용 예시__ 와 __Jvault 확장__ 과 같은 라이브러리에 대한 더 자세한 설명은 [Learn Jvault]() 를 참조하세요.
...
@InternalBean어노테이션으로 마킹된 클래스는 Vault(빈 저장소)에 등록되며, DI 과정중 자신이 허가하지 않은 클래스가 자신을 주입받으려고 시도한다면, 예외를 던집니다.   
...
따라서, 이 목차에서는 @InternalBean이 위치한 경로지정부터 생성될 Vault의 정보를 설정하는 방법과 Vault를 이용해 의존성을 주입하는 방법을 알아볼것 입니다.   
...
ClassVault vault = vaultFactory.get(buildInfo);
...
만약, 한번 이라도 VaultFactory에 의해 생성된 Vault라면, 다음과 같이 이름을 통해 조회할 수 있습니다.   
   
``` Java
ClassVaultFactory vaultFactory = ClassVaultFactory.getInstance();

ClassVault vault = vaultFactory.get("CAR_VAULT");

Car car = vault.inject(Car.class);
```
   
> TIP : 앞서, SquareWheel Bean을 SINGLETON으로 등록한것을 상기하세요. 위 예제에서 생성되는 car과 이전 예제에서 생성한 car은 서로 다른 객체이지만, Car 내부에 주입된 SquareWheel은 같은 객체 입니다.
   
  
다음은, Class를 이용한 설정 예시입니다.   
Vault설정 클래스는 클래스 이름 위에 @BeanArea를 마킹함으로 정의할 수 있습니다.
